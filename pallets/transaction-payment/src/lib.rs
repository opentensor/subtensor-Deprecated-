...
mod tests {
	...
	fn signed_extension_transaction_payment_work() {
		...
		{
			let len = 10;
			let pre = ChargeTransactionPaymentOld::<Runtime>::from(0)
				.pre_dispatch(&1, CALL, &info_from_weight(5), len)
				.unwrap();
			assert_eq!(Balances::free_balance(1), 100 - 5 - 5 - 10);

			assert!(
				ChargeTransactionPaymentOld::<Runtime>
					::post_dispatch(pre, &info_from_weight(5), &default_post_info(), len, &Ok(()))
					.is_ok()
			);
			assert_eq!(Balances::free_balance(1), 100 - 5 - 5 - 10);

			let pre = ChargeTransactionPaymentOld::<Runtime>::from(5 /* tipped */)
	...
	fn signed_extension_transaction_payment_is_bounded() {
			...
			.execute_with(||
		{
			...
			assert!(
				ChargeTransactionPaymentOld::<Runtime>::from(0)
	...
	fn signed_ext_length_fee_is_also_updated_per_congestion() {
			...
			.execute_with(||
		{
			...
			assert!(
				ChargeTransactionPaymentOld::<Runtime>::from(10) // tipped
	...
	fn refund_consistent_with_actual_weight() {
			...
			.execute_with(||
		{
			...
			let pre = ChargeTransactionPaymentOld::<Runtime>::from(tip)
